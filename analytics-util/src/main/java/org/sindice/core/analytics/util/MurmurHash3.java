...
class MurmurHash3 {
   ...
   * @return 128 bit hashed key, in an array containing two longs
   */
  public static long[] MurmurHash3_x64_128(final byte[] key, final int off, final int len, final int seed) {
    ...
    for (int i = 0; i < len / 16; i++) {
      state.k1 = getblock(key, off + i * 2 * 8);
      state.k2 = getblock(key, off + (i * 2 + 1) * 8);
    ...
    final int tail = (len >>> 4) << 4;

    switch (len & 15) {
      case 15:
        state.k2 ^= (long) key[off + tail + 14] << 48;
      case 14:
        state.k2 ^= (long) key[off + tail + 13] << 40;
      case 13:
        state.k2 ^= (long) key[off + tail + 12] << 32;
      case 12:
        state.k2 ^= (long) key[off + tail + 11] << 24;
      case 11:
        state.k2 ^= (long) key[off + tail + 10] << 16;
      case 10:
        state.k2 ^= (long) key[off + tail + 9] << 8;
      case 9:
        state.k2 ^= (long) key[off + tail + 8] << 0;

      case 8:
        state.k1 ^= (long) key[off + tail + 7] << 56;
      case 7:
        state.k1 ^= (long) key[off + tail + 6] << 48;
      case 6:
        state.k1 ^= (long) key[off + tail + 5] << 40;
      case 5:
        state.k1 ^= (long) key[off + tail + 4] << 32;
      case 4:
        state.k1 ^= (long) key[off + tail + 3] << 24;
      case 3:
        state.k1 ^= (long) key[off + tail + 2] << 16;
      case 2:
        state.k1 ^= (long) key[off + tail + 1] << 8;
      case 1:
        state.k1 ^= (long) key[off + tail + 0] << 0;
        bmix(state);
    }
  ...
  public static long hashLong(byte[] payload) {
  ...
  public static long hashLong(long[] payload) {
    return MurmurHash3_x64_64(payload, 9001);
  }
  ...
  public static long hashLong(long hashcode) {
    byte[] b = new byte[8];
    b[0] = (byte) hashcode;
    b[1] = (byte) (hashcode >>> 8);
    b[2] = (byte) (hashcode >>> 16);
  ...
  public static long hashLong(Object o) {
    if (o instanceof byte[])
