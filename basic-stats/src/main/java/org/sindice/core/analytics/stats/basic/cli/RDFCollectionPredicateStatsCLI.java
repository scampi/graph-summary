...
public class RDFCollectionPredicateStatsCLI
extends AbstractAnalyticsCLI {
  ...
  protected void initializeOptionParser(final OptionParser parser) {
    parser.accepts(DATE, "Date in the form YYYY-MM-DD").withRequiredArg().describedAs("Date").ofType(String.class);
  }
  ...
  throws Exception {
    ...
    AppProps.setApplicationJarClass(properties, RDFCollectionPredicateStatsCLI.class);

    final Tap source = new Hfs(getInputScheme(Analytics.getHeadFields(RDFCollectionPredicateStats.class)), input.get(0));
    final Tap sink = new Hfs(new ExtensionTextLine("nq", Compress.ENABLE), output.get(0), SinkMode.REPLACE);
    final RDFCollectionPredicateStats pipe = new RDFCollectionPredicateStats();
    final Flow<?> flow = new HadoopFlowConnector(properties).connect(Analytics.getName(pipe), source, sink, pipe);
  ...
  throws Exception {
    final RDFCollectionPredicateStatsCLI cli = new RDFCollectionPredicateStatsCLI();
