 ...
 */
public class RDFCollectionNamespaceFunction
extends AbstractRDFCollectionFunction {

  private static final long serialVersionUID = 5038737465393537157L;

  public RDFCollectionNamespaceFunction(final Fields fieldDeclaration) {
  ...
  public void operate(FlowProcess flowProcess,
    ...
    // Get the fields
    final String namespace = args.getString("namespace");
    ...
    final Long hash = Hash.getHash64(namespace);
    final URI bnode = cxt.value.createURI(CollectionAnalyticsVocab.ANY23_PREFIX,
      hash.toString().replace('-', 'n'));

    final Value nsLit = cxt.value.createLiteral(namespace);

    final Literal objectsNumberLit = cxt.value.createLiteral(objectsNumber);
    final Literal urlLit = cxt.value.createLiteral(url);
    final Literal domainLit = cxt.value.createLiteral(domain);
    final Literal sndDomainList = cxt.value.createLiteral(sndDomain);

    super.writeStatement(cxt, functionCall.getOutputCollector(), bnode, cxt.statsName, cxt.colNSStat, cxt.graphName);
    super.writeStatement(cxt, functionCall.getOutputCollector(), bnode, cxt.label, nsLit, cxt.graphName);
    super.writeStatement(cxt, functionCall.getOutputCollector(), bnode, cxt.nUrls, objectsNumberLit, cxt.graphName);
    super.writeStatement(cxt, functionCall.getOutputCollector(), bnode, cxt.nUrls, urlLit, cxt.graphName);
