 ...
 */
public class BasicPredicateStatsCLI
extends AbstractAnalyticsCLI {
  ...
  throws Exception {
    ...
    AppProps.setApplicationJarClass(properties, BasicPredicateStatsCLI.class);
    ...
    final BasicPredicateStats pipe = new BasicPredicateStats();
    final Fields fd = new Fields("predicate", "objectsNumber", "url", "domain", "sndDomain");
    final Flow<?> flow = new HadoopFlowConnector(properties)
    .connect("basic-predicate-stats", source, new Hfs(new SequenceFile(fd), output.get(0), SinkMode.REPLACE), pipe);
  ...
  throws Exception {
    final BasicPredicateStatsCLI cli = new BasicPredicateStatsCLI();
