...
import org.sindice.core.analytics.stats.distribution.assembly.DistributionLiteralLength;
 ...
 */
public class DistributionLiteralLengthCLI extends AbstractAnalyticsCLI {

  private static final Logger logger = LoggerFactory
      .getLogger(DistributionLiteralLengthCLI.class);
  ...
  throws Exception {
    ...
    AppProps.setApplicationJarClass(properties,
        DistributionLiteralLengthCLI.class);

    // Set pipe and flow
    final DistributionLiteralLength dpt = new DistributionLiteralLength();

    final Tap source = new Hfs(
        getInputScheme(Analytics.getHeadFields(dpt)), input.get(0));
    final Tap sink = new Hfs(new SequenceFile(Analytics.getTailFields(dpt)),
      output.get(0), SinkMode.REPLACE);

    final Flow<?> flow = new HadoopFlowConnector(properties).connect(
      Analytics.getName(dpt), source, sink, dpt);
  ...
  public static void main(final String[] args) throws Exception {
    final DistributionLiteralLengthCLI cli = new DistributionLiteralLengthCLI();
