...
extends AbstractAnalyticsCLI {
  ...
  throws Exception {
    ...
    AppProps.setApplicationJarClass(properties, BasicUriStatsCLI.class);
    ...
    final BasicUriStats uriStats = new BasicUriStats();

    final Fields fieldsDeclaration = new Fields("uri", "total", "graph", "domain-freq", "snd-level-domain-freq");
    final Flow<?> flow = new HadoopFlowConnector(properties)
    .connect("basic-uri-stats", source, new Hfs(new SequenceFile(fieldsDeclaration), output.get(0), SinkMode.REPLACE), uriStats);
  ...
  throws Exception {
    final BasicUriStatsCLI cli = new BasicUriStatsCLI();
