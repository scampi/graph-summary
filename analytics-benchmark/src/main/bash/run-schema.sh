...
function _usage() {
cat <<EOF
    Welcome to the Data Graph Summary benchmark CLI - Schema Precision.
...
for dataset in $DATASETS; do
    for algo in $ALGORITHMS; do
        ...
        echo "Running the schema precision on [$path]"
        ...
        # Run the Schema precision
        if !($HADOOP_BIN fs $HADOOP_CLUSTER -test -d $path/precision/schema); then
            echo "Run SchemaAssemblyCLI"
            $HADOOP_BIN jar $JAR org.sindice.analytics.benchmark.cascading.precision.schema.SchemaAssemblyCLI \
                        $HADOOP_CLUSTER $D_OPTIONS \
                        --cascade-config $CONFIG_YAML \
