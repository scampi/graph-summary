...
/**
 * Parse data from Relations branch and create associated edge N-triples in the
 * output RDF
 * 
 * @author Pierre Bailly-Ferry <pierre.bailly@deri.org>
 */
@AnalyticsName(value="RDF-RelationsGraph-Summary")
@AnalyticsHeadPipes(values={
  @AnalyticsPipe(from=RelationsGraph.class)
})
@AnalyticsTailPipes(values={
  @AnalyticsPipe(fields={ "rdf" })
})
public class RDFRelationsGraph extends AnalyticsSubAssembly {

  private static final long  serialVersionUID  = -4355735413534565104L;

  public RDFRelationsGraph() {
    super();
  }

  public RDFRelationsGraph(Pipe[] pipe) {
  ...
  protected Object assemble(Pipe[] previous, Object... args) {
    ...
    pipe = new Each(pipe, Analytics.getHeadFields(this), new RDFRelationsGraphFunction(Analytics.getTailFields(this)));
    pipe = new Unique(pipe, Analytics.getTailFields(this));
